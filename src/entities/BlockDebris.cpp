...
BlockDebris::BlockDebris(const sf::Vector2f& position, const sf::Vector2f& velocity)
    : Entity(position, sf::Vector2f(DEBRIS_SIZE, DEBRIS_SIZE)),
      m_vel(velocity) {
    setSprite(DEBRIS_TEXTURE_PATH);
    m_animationSystem->addAnimation("idle",
        AnimationSystem::createManualAnimation(std::vector<sf::IntRect>{sf::IntRect({16, 0}, {8, 8})}, 1.f));
    playAnimation("idle");
}
