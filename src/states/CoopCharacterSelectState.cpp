/**
 * @file CoopCharacterSelectState.cpp
 * @brief Sequential two-player character selection for the co-op campaign.
 */
...
const sf::Color P2_COLOR(120, 255, 150);
...
CoopCharacterSelectState::CoopCharacterSelectState(int selectedLevel)
    : m_selectedLevel(selectedLevel) {}

void CoopCharacterSelectState::initBackdropPanel() {
...
void CoopCharacterSelectState::initCards() {
    // Mario Card (Left)
    ...
    m_p2BadgeBox.setFillColor(sf::Color(10, 60, 30));
    m_p2BadgeBox.setOutlineColor(P2_COLOR);
    m_p2BadgeBox.setOutlineThickness(1.5f);
}

void CoopCharacterSelectState::initAvatars() {
...
void CoopCharacterSelectState::initTextLabels() {
    if (!m_fontLoaded) return;

    m_titleText.emplace(m_font, "2 PLAYER CO-OP", TITLE_FONT_SIZE);
    ...
    m_subtitleText.emplace(m_font, "PLAYER 1 - CHOOSE YOUR HERO!", SUBTITLE_FONT_SIZE);
...
void CoopCharacterSelectState::initMenu() {
    rebuildMenuForPhase();
}

void CoopCharacterSelectState::rebuildMenuForPhase() {
    if (m_subtitleText) {
        if (m_phase == Phase::PLAYER_ONE) {
            ...
            std::string splash = "P1 " + std::string(characterName(m_playerOneChoice)) +
                                 "  &  " + std::string(characterName(m_playerTwoChoice)) + " P2 - TEAM UP!";
...
void CoopCharacterSelectState::confirmChoice(CharacterType characterType) {
    ...
    if (m_phase == Phase::PLAYER_ONE) {
        ...
        m_selectedIndex = 0; // reset cursor for P2
        rebuildMenuForPhase();
    } else if (m_phase == Phase::PLAYER_TWO) {
...
void CoopCharacterSelectState::queueMatch() {
    if (m_transitioning) return;

    m_transitioning = true;
    GameManager::getInstance().changeState(
        std::make_unique<PlayState>(m_selectedLevel, m_playerOneChoice, m_playerTwoChoice));
}

void CoopCharacterSelectState::onEnter() {
...
void CoopCharacterSelectState::onExit() {}

void CoopCharacterSelectState::processEvents(const sf::Event& event) {
...
void CoopCharacterSelectState::processInput(const InputState& inputState) {
    if (m_transitioning) return;
...
void CoopCharacterSelectState::update(float dt) {
    m_animTimer += dt;
    ...
    if (m_phase == Phase::PLAYER_ONE) {
        ...
        if (m_p1BadgeText) {
            m_p1BadgeText->setPosition({targetX + 13.f, CARD_Y - 7.f});
        }
    } else if (m_phase == Phase::PLAYER_TWO) {
        // P1 fixed badge on chosen card
        ...
        if (m_p1BadgeText) {
            m_p1BadgeText->setPosition({p1X + 13.f, CARD_Y - 7.f});
        }
        ...
        if (m_p2BadgeText) {
            m_p2BadgeText->setPosition({p2X + 13.f, CARD_Y - 7.f});
        }
...
void CoopCharacterSelectState::render(sf::RenderTarget& target) {
