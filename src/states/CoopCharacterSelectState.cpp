 ...
 *        Modeled on PvpCharacterSelectState; ESC returns to the co-op stage
 *        select instead of the main menu.
 */
...
void CoopCharacterSelectState::queueMatch() {
    ...
    GameManager::getInstance().changeState(
        std::make_unique<PlayState>(m_selectedLevel, m_playerOneChoice,
                                    m_playerTwoChoice));
}
